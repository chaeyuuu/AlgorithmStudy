...
# 아이디어
# 국가번호 k의 등수만 구하면되기때문에 거기에 포인트를 잡고 할 것 -> 이차원 배열로 입력받고 k와 같은 국가번호 찾기 -> 동점자 있으면 rank 증가
...
# for i in range(N):
#     if K == data[i][0]:
#         k_g, k_s, k_b = data[i][1], data[i][2], data[i][3]
#         break
    
# for i in range(N):
#     c_g, c_s, c_b = data[i][1], data[i][2], data[i][3]
#     if (c_g > k_g) or (c_g == k_g and c_s > k_s) or (c_g == k_g and c_s == k_s and c_b > k_b):
#         rank += 1
#       # 동점자가 있으면 순위변동 x
#     elif (c_g == k_g) and (c_s== k_s) and (c_b== k_b):
#         continue
#     else:
#         break
